...
const Home = () => {
  ...
  useEffect(() => {
    fetch("https://playground.4geeks.com/todo/users/lpardo")
      .then((res) => res.json())
      .then((data) => {
        setTodos(data.todos);
      })
  ...
  const agregarTarea = async (nuevaTarea) => {
    try {
      await fetch("https://playground.4geeks.com/todo/todos/lpardo", {
        method: "POST",
      ...
      // Actualizar lista
      const res = await fetch("https://playground.4geeks.com/todo/users/lpardo");
  ...
  const borrarTodas = async () => {
    try {
      await fetch("https://playground.4geeks.com/todo/users/lpardo", {
        method: "DELETE"
      });
      setTodos([]);
    } catch (error) {
